...
class TextClassifier(nn.Module):
    ...
    def validation(self,valid_dataloader, replaced_dataloader,embedding_sens_eval_dataloader, epoch,device,save=False):
        ...
        if(self.sensitivity_method == 'word'):
            ...
            sensitivity, mean_sensitivity = embedding_label_sensitivity(embedding_sens_eval_dataloader, self, self.model.roberta.embeddings, device, self.replace_size, self.embedding_noise_variance)
        elif self.sensitivity_method =='sentence_embedding':# we use a subset of training data to test the sensitivity
            sensitivity, mean_sensitivity = sentence_embedding_label_sensitivity(embedding_sens_eval_dataloader, self, self.model.roberta.embeddings, device, self.replace_size, self.embedding_noise_variance)
